...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    factory(exports, require('./ui_utils.js'), require('./download_manager.js'),
      ...
      require('./pdf_document_properties.js'), require('./product_popup.js'),
  ...
  var PDFPublisherPopup = pdfPublisherPopupLib.PDFPublisherPopup;
  ...
  var PDFViewerApplication = {
    ...
    /** @type {PDFPublisherPopup} */
    pdfPublisherPopup: null,
    /** @type {PDFLinkService} */
    pdfLinkService: null,
    /** @type {PDFHistory} */
    pdfHistory: null,
    ...
    load: function pdfViewLoad(pdfDocument, scale) {
      ...
      this.pdfPublisherPopup.setDocumentAndUrl(pdfDocument, this.url);
    ...
    bindEvents: function pdfViewBindEvents() {
      ...
      eventBus.on('publisherinfo', webViewerPublisherPopup);
  ...
  function extraUIConfig() {
    ...
    var eventBus = appConfig.eventBus || getGlobalEventBus();
    var valid_metadata = function (key) {
      var value = matadataConfig[key];
    ...
    if (valid_metadata("allow_download")) {
      appConfig.toolbar.download.classList.remove('hidden');
    }

    if (valid_metadata("allow_favorite")) {
      appConfig.sidebar.favoriteButton.classList.remove('hidden');
    }

    if (valid_metadata("allow_fullscreen")) {
      appConfig.toolbar.presentationModeButton.classList.remove('hidden');
    }

    if (valid_metadata("allow_share")) {
  ...
  function webViewerPublisherPopup() {
    PDFViewerApplication.pdfPublisherPopup.open();
  }
  ...
  function webViewerClick(evt) {
    ...
    var secondaryToolbar = PDFViewerApplication.secondaryToolbar;
    var pdfProductPopup = PDFViewerApplication.pdfProductPopup;
    ...
    if (pdfProductPopup._isPopupOpen()) {
      openContainer = true;
    }

    if (publisherPopup._isPopupOpen()) {
      openContainer = true;
    }

    if (shareToolbar.isOpen) {
      openContainer = true;
    }

    if (pageViewsToolbar.isOpen) {
      openContainer = true;
    }
    if (!openContainer) {
      return;
    ...
    if (publisherPopup._isPopupOpen() && appConfig.publisherPopup.container == evt.target &&
      evt.target !== appConfig.publisherPopup.publisherPopup) {
      publisherPopup.close();
    }

    if (pdfProductPopup._isPopupOpen() && appConfig.productPopup.container == evt.target &&
        evt.target !== appConfig.productPopup.productPopup) {
      pdfProductPopup.close();
    }
  }
